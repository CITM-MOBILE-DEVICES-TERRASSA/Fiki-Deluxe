...
public class EnemyBird : MonoBehaviour
{
    ...
    private void Update()
    {
        // Movimiento hacia el jugador o en la dirección inicial
        if (!diving)
        {
            transform.position += flyDirection * speed * Time.deltaTime;

            // Despawnear el pájaro si está demasiado lejos del jugador (o de la escena)
            if (Vector3.Distance(transform.position, playerPosition) > 20f) // Cambia 20f según tu escena
            {
                Destroy(gameObject);
            }
        }
    ...
    {
        // Activar la animación de picado (si existe)
        yield return new WaitForSeconds(diveDuration);

        // Determinar la dirección de empuje
        Vector2 pushDirection = (player.position - transform.position).normalized;

        // Llamar al método `ApplyPush` en el jugador para actualizar su posición objetivo
